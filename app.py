...
def connection():    
    server = 'nitya.database.windows.net'
    database = 'nitya'
    username = 'nitya'
    password = 'Nhk@241098'
...
def form2():
    ...
    if form.validate_on_submit():
        try:
            ...
            cursor.execute("SELECT * FROM earthquake where time > ? AND mag BETWEEN ? AND ?", days_ago, r1, r2)
            result = []
            while True:
                row = cursor.fetchone()
                if not row:
...
def form4():
    if request.method == 'POST':
        try:
            ...
            cursor.execute("SELECT * FROM earthquake where type = ?", clus)
            result = []
            while True:
                row = cursor.fetchone()
                if not row:
